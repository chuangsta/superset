...
Flask-SQLAlchemy 2.5.x imports it at module level and uses ``.top`` to
store per-app-context query recordings. Superset stays on
Flask-SQLAlchemy 2.5.x (and SQLAlchemy < 2), so this module re-exposes a
lightweight ``flask._app_ctx_stack`` whose ``.top`` delegates attribute
storage to ``flask.g`` — the canonical per-app-context namespace in
Flask 3.x.
