...
- **New config flag `SSH_TUNNEL_REJECT_SHA1_KEYS` (default `True`).** Mitigation for CVE-2026-44405 (paramiko still permits the SHA-1 `ssh-rsa` signature algorithm in `rsakey.py`; no upstream fix published). When enabled, the host-key verification probe that Superset opens directly (`superset/extensions/ssh.py`) disables `ssh-rsa` via paramiko's `disabled_algorithms`, forcing `rsa-sha2-256` / `rsa-sha2-512`. Operators that must verify a host key against a legacy SSH server offering only SHA-1 RSA signatures can set it to `False`. **Known/accepted residual risk:** this only hardens the probe. The tunnel that carries traffic is opened by `sshtunnel` (`>=0.4.0,<0.5`), whose `SSHTunnelForwarder` builds its own `paramiko.Transport` and exposes no `disabled_algorithms` hook, so SHA-1 cannot be rejected on that connection through configuration — re-check on the next paramiko / sshtunnel release.
