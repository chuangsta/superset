...
- **New config `SSH_TUNNEL_DISABLED_ALGORITHMS` (SHA-1 `ssh-rsa` disabled by default).** To mitigate CVE-2026-44405 / GHSA-r374-rxx8-8654 (Paramiko allowing the SHA-1 algorithm), Superset now passes `disabled_algorithms` to the paramiko `Transport` it opens for the SSH-tunnel **host-key verification probe**, defaulting to rejecting the legacy `ssh-rsa` (SHA-1) algorithm; modern servers negotiate `rsa-sha2-256` / `rsa-sha2-512` instead. This only affects tunnels that use host-key verification (an expected `server_host_key` is set, or `SSH_TUNNEL_STRICT_HOST_KEY_CHECKING` is enabled). If such a tunnel targets a legacy SSH server that only offers `ssh-rsa`, set `SSH_TUNNEL_DISABLED_ALGORITHMS = {}` in `superset_config.py` to restore the previous behavior.
  - **Accepted/monitored residual risk:** the tunnel connection itself is opened by the `sshtunnel` library, whose public API constructs the paramiko `Transport` internally and rejects unknown kwargs, so `disabled_algorithms` cannot be forwarded to it without depending on private internals. Rather than pin around this with a fragile hack, the tunnel connection is documented as an accepted, monitored risk. Severity is Low: exploiting SHA-1 here requires an active MITM on an operator-controlled SSH path. Re-check on the next Paramiko/sshtunnel release for an upstream fix (a published patch removing SHA-1, or a supported way to pass `disabled_algorithms` through `sshtunnel`).
